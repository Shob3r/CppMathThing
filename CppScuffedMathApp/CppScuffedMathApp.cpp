//C++ libs that are in the standard library/are pre-installed
#include <iostream>
#include <math.h>
#include <cmath>
#include <io.h>
...
//C++ libraries that are not in the standard library/pre-installed, and will need to be installed on other systems before compiling
...
int main() {
	...
	srand(time(0)); //makes it so that the program generates a different random number each time someone launches the program
...
mainMenu:
	...
	if (menuChoice == 1) {
		...
		if (CalculatorNum1 > CalculatorNum2) {
			cout << "Subtracting the smaller number from the larger one is: " << CalculatorNum1 - CalculatorNum2 << std::endl;
		}
		else {
			cout << "Subtracting the smaller number from the larger one is: " << CalculatorNum2 - CalculatorNum1 << std::endl;
		}
